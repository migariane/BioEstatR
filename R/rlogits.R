#' @title Regresion logistica simple
#' @description Ajuste del modelo de regresion logistica simple de acuerdo a la especificacion y ~ x
#' @param f formula: especificacion del modelo (ej. y ~ x)
...
#' @return Informe con medidas de asociacion (OR), estimacion de los parametros de regr. logistica, bondad de ajuste (Hosmer-Lemeshow) y curva ROC.
...
rlogits <- function(f, data = NULL, pred = NULL, grf = FALSE, alfa = 0.05, conf = 1 - alfa, decs = 3) {
  
  if (missing(f)) stop("Debe especificar una formula para el modelo.")
  if (!inherits(f, "formula")) stop("El primer argumento debe ser una formula (ej. y ~ x).")
